...
export async function getWorkbench(role: AppRole, bundle: ReportingBundle): Promise<WorkbenchResult> {
  ...
  if (!isGroup && !isKitchenManager) {
    return {
      items: [],
      allClear: true,
      clearMessage: "No operational actions are assigned to this access role.",
    };
  }
  ...
  const [checkboard, actions, managers, reviews] = await Promise.all([
  ...
  if (reportsDue.length) {
    items.push({
      ...
      title: `${plural(reportsDue.length, "report")} not submitted`,
      detail: `${reportsDue.map((site) => site.name).join(", ")} · w/c ${formatDate(bundle.week.start)}`,
      href: isGroup ? "/reports" : "/reports/new",
      cta: isGroup ? "Open reports" : "Finish report",
    });
  }

  if (isGroup && awaitingApproval.length) {
    items.push({
      key: "approvals",
      tone: "warn",
      count: awaitingApproval.length,
      title: `${plural(awaitingApproval.length, "report")} waiting for approval`,
      detail: awaitingApproval.map((report) => report.siteName).join(", "),
      href: "/approvals",
      cta: "Open approvals",
    });
  }
  ...
  const openRunKeys = new Set(
    checkboard.runs
      .filter((run) => run.status === "draft" || run.status === "reopened")
      .map((run) => `${run.templateId}:${run.periodStart}`),
  );
  ...
  if (openRunKeys.size) {
    items.push({
      key: "checks",
      tone: "warn",
      count: openRunKeys.size,
      title: `${plural(openRunKeys.size, "kitchen check")} due or open`,
      detail: "Current daily and weekly checks still need completion",
      href: "/checks",
      cta: "Open checks",
    });
  }
  ...
  if (isGroup) {
    ...
    if (submittedChecks.length) {
      items.push({
        key: "check-review",
        tone: "warn",
        count: submittedChecks.length,
        title: `${plural(submittedChecks.length, "check")} waiting for review`,
        detail: submittedChecks.map((run) => run.siteName).join(", "),
        href: "/checks",
        cta: "Review checks",
      });
    }
  ...
  if (overdueActions.length) {
    items.push({
      ...
      title: `${plural(overdueActions.length, "action")} overdue`,
      detail: isGroup ? "Across the manager action log" : "Your outstanding action log",
      href: "/performance/actions",
      cta: "Open action log",
    });
  } else if (isKitchenManager && openActions.length) {
    items.push({
      key: "open-actions",
      tone: "ok",
      count: openActions.length,
      title: `${plural(openActions.length, "action")} in progress`,
      detail: "Nothing overdue — update progress as work is completed",
      href: "/performance/actions",
      cta: "Update actions",
    });
  }
  ...
  if (isGroup) {
    const completedManagerIds = new Set(
      reviews
        .filter((review) => review.weekCommencing === bundle.week.start && ["finalised", "acknowledged"].includes(review.status))
        .map((review) => review.managerId),
    );
    ...
    if (waiting.length) {
      items.push({
        ...
        title: `${plural(waiting.length, "1-1", "1-1s")} to complete this week`,
        detail: waiting.map((manager) => manager.fullName.split(" ")[0]).join(", "),
        href: "/one-to-ones",
        cta: "Open 1-1s",
      });
    }
    ...
    if (pendingAcknowledgement) {
      items.push({
        key: "acknowledge-one-to-one",
        tone: "warn",
        count: null,
        title: "1-1 ready to acknowledge",
        detail: `Week commencing ${formatDate(pendingAcknowledgement.weekCommencing)}`,
        href: `/one-to-ones/${pendingAcknowledgement.id}`,
        cta: "Read and acknowledge",
      });
    }
  }
  ...
  return {
    items,
    allClear,
    clearMessage: isGroup
      ? "Reports are submitted, checks are clear, approvals are handled and this week’s 1-1s are complete."
      : "Your report and checks are complete, with no overdue actions or 1-1 acknowledgement waiting.",
  };
}

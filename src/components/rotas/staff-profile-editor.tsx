...
export function RotaCloudSync({ configured }: { configured: boolean }) {
  ...
  return <form action={action} className="rota-sync"><div><strong>{configured ? "RotaCloud read-only sync is configured" : "RotaCloud is not connected"}</strong><span>{configured ? "Imports people, hourly wages, roles, locations and contracted hours. Salaries remain controlled in Labour settings." : "Add ROTACLOUD_API_KEY in the server environment. Manual profiles work without it."}</span></div><button className="button button--secondary" disabled={pending || !configured} type="submit"><CloudDownload aria-hidden="true" size={16} />{pending ? "Reading RotaCloud…" : "Sync team from RotaCloud"}</button>{state.status !== "idle" ? <p className={`form-message ${state.status === "error" ? "form-message--error" : "form-message--success"}`} role={state.status === "error" ? "alert" : "status"}>{state.message}</p> : null}</form>;
}
...
export function StaffProfileEditor({
  ...
  const [siteId, setSiteId] = useState(staff?.siteId ?? sites[0]?.id ?? "");
  const router = useRouter();
  useEffect(() => { if (state.status === "success") router.refresh(); }, [router, state.status]);
  ...
  const orderedAccounts = useMemo(() => [...appProfiles].sort((a, b) => {
    const scopeRank = (profile: RotaAppProfileOption) => profile.siteIds.includes(siteId) ? 0 : profile.siteIds.length === 0 ? 1 : 2;
    return scopeRank(a) - scopeRank(b) || a.name.localeCompare(b.name) || a.role.localeCompare(b.role);
  }), [appProfiles, siteId]);

  const accountScope = (profile: RotaAppProfileOption) => {
    if (profile.siteIds.includes(siteId)) return "this kitchen";
    if (!profile.siteIds.length) return "group account";
    return "another kitchen";
  };
  ...
  return (
    <details className={`staff-profile panel ${staff ? "" : "staff-profile--new"}`} open={!staff}>
      <summary><div className="staff-profile__identity">{staff ? <><strong>{staff.staffName}</strong><span>{staff.roleTitle} · {sites.find((site) => site.id === staff.siteId)?.name ?? "Kitchen"}</span></> : <><strong><Plus aria-hidden="true" size={15} /> Add staff member</strong><span>Link their app account and rota constraints once</span></>}</div>{staff ? <div className="staff-profile__summary"><span>{staff.appProfileId ? <><Link2 size={13} /> Account linked</> : "No account link"}</span><span>{staff.targetWeeklyHours}h target</span><span>{staff.payBasis === "hourly" ? `£${staff.hourlyRate?.toFixed(2)}/h` : "Salary from Labour settings"}</span></div> : null}</summary>
      <form action={action} className="report-form staff-profile__form">
        ...
        <div className="form-grid form-grid--three">
          <label className="field field--wide"><span className="field__label">Linked app account</span><select className="field__input" name="appProfileId" value={appProfileId} onChange={(event) => chooseAccount(event.target.value)}><option value="">Not linked yet</option>{orderedAccounts.map((profile) => { const usedElsewhere = Boolean(profile.linkedStaffId && profile.linkedStaffId !== staff?.id); return <option disabled={usedElsewhere} key={profile.id} value={profile.id}>{profile.name} · {roleLabel(profile.role)} · {accountScope(profile)}{usedElsewhere ? " · already linked" : ""}</option>; })}</select><span className="field__hint">Accounts assigned to the selected kitchen appear first. Preview environments only show accounts created in that environment.</span></label>
          <label className="field"><span className="field__label">Kitchen allocation</span><select className="field__input" name="siteId" onChange={(event) => setSiteId(event.target.value)} required value={siteId}>{sites.map((site) => <option key={site.id} value={site.id}>{site.name}</option>)}</select></label>
        ...
        <div className="form-grid form-grid--two staff-profile__scope">
          <label className="check-card"><input defaultChecked={staff?.organisationWide ?? false} name="organisationWide" type="checkbox" value="true" /><span><strong>Show in every kitchen rota</strong><small>For group staff such as Group Chef or development managers. Salary allocation still comes only from Labour settings.</small></span></label>
          <label className="check-card"><input defaultChecked={staff?.primarySite ?? true} name="primarySite" type="checkbox" value="true" /><span><strong>Primary kitchen allocation</strong><small>Marks this as the person’s main role and scheduling allocation.</small></span></label>
        </div>
        ...
        <div className="form-divider"><span><LockKeyhole aria-hidden="true" size={14} /> Labour cost source</span></div>
        <div className="form-grid form-grid--three">
          ...
          {payBasis === "hourly" ? <label className="field"><span className="field__label">Hourly wage</span><div className="input-prefix"><span>£</span><input className="field__input" defaultValue={staff?.hourlyRate ?? ""} min="0.01" name="hourlyRate" required step="0.01" type="number" /></div><span className="field__hint">Normally imported from RotaCloud.</span></label> : <label className="field"><span className="field__label">Contracted weekly hours</span><input className="field__input" defaultValue={staff?.contractedWeeklyHours ?? 40} min="0.25" name="contractedWeeklyHours" required step="0.25" type="number" /></label>}
        </div>
        {payBasis === "salaried" ? <div className="privacy-callout">Salary, on-cost and site split are not entered here. The rota reads the existing allocation from <strong>Labour settings</strong> once, then adds scheduled hourly labour. Adding or removing a salaried person’s shift does not add their salary again.</div> : null}

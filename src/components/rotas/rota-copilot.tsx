...
const suggestedQuestions = [
  "Can I give Scott Saturday off?",
  "What if sales are 15% higher?",
  "What is the cheapest compliant rota?",
  "Why did you schedule Warren here?",
  "Could one chef cover lunch?",
];
...
export function RotaCopilot({ plan, signals, staffTargets, initialReview }: Props) {
  ...
  const intelligence = useMemo(() => {
    ...
    const underCover = plan.days.flatMap((day) => day.coverage.map((slot) => ({ ...slot, date: day.businessDate }))).filter((slot) => slot.assigned < slot.required);
    const labourPct = plan.forecastSales > 0 ? plan.plannedCost / plan.forecastSales * 100 : 0;
    const staffHours = staffTargets.map((staff) => ({
      ...staff,
      plannedHours: shifts.filter((shift) => shift.staffProfileId === staff.id).reduce((sum, shift) => sum + shift.paidMinutes / 60, 0),
    }));
    ...
    if (labourPct > plan.labourTargetPct) score -= Math.min(15, Math.ceil((labourPct - plan.labourTargetPct) * 3));
    score = Math.max(0, Math.round(score));

    const strengths: string[] = [];
    if (labourPct <= plan.labourTargetPct) strengths.push(`Labour is within the ${plan.labourTargetPct.toFixed(1)}% target.`);
    ...
    if (!unsafeBreaks.length) strengths.push("Planned breaks have a suggested low-risk window.");
    if (!unfilled.length) strengths.push("All generated shifts have an assigned team member.");
    ...
    if (labourPct > plan.labourTargetPct) risks.push(`Labour is ${(labourPct - plan.labourTargetPct).toFixed(1)} points above target.`);
    ...
    if (underCover.length) recommendations.push(`Fix the ${formatDate(underCover[0].date)} ${underCover[0].slotTime} cover gap before reducing any other shift.`);
    if (belowMinimum.length) recommendations.push(`Add hours for ${belowMinimum[0].name} in a genuine demand window rather than creating a standalone short shift.`);
    ...
    if (unsafeBreaks.length) recommendations.push("Manually place the breaks with no safe suggested window and recheck live cover.");
    if (!recommendations.length) recommendations.push("Run a final service stress-test at +15% sales, then approve the rota if cover still holds.");

    return { score, labourPct, staffHours, strengths: strengths.slice(0, 5), risks: risks.slice(0, 5), recommendations: recommendations.slice(0, 4) };
  ...
  const safeContext = useMemo(() => ({
    ...
    days: plan.days.map((day) => {
      ...
      return {
        ...
        coverageShortfalls: day.coverage.filter((slot) => slot.assigned < slot.required).map((slot) => ({
          time: slot.slotTime,
        ...
        shifts: day.shifts.map((shift, index) => {
          ...
          return {
            ...
            suggestedBreak: suggestedBreak?.startTime && suggestedBreak.endTime ? `${suggestedBreak.startTime}-${suggestedBreak.endTime}` : null,
          };
        }),
      };
    }),
  ...
  return (
    <section aria-labelledby="operations-intelligence-title" className="operations-intelligence panel">
      <header className="operations-intelligence__header">
        <div className="operations-intelligence__score" aria-label={`Operations Intelligence score ${intelligence.score} out of 100`}>
          <span>{intelligence.score}</span><small>/100</small>
        </div>
        <div>
          <p className="page-header__eyebrow">Operations Intelligence</p>
          <h2 id="operations-intelligence-title">A senior manager’s review of this rota</h2>
          <p>The score comes from actual cover, cost, shift-length and agreed-hours checks. AI explains and challenges the plan; it does not override hard rules.</p>
        </div>
        <span className={`operations-intelligence__status operations-intelligence__status--${intelligence.score >= 85 ? "good" : intelligence.score >= 65 ? "watch" : "risk"}`}>
          ...
          {intelligence.score >= 85 ? "Strong starting point" : intelligence.score >= 65 ? "Manager changes needed" : "Not ready to approve"}
        </span>
      </header>
      ...
      <div className="operations-intelligence__grid">
        ...
        <InsightColumn icon={AlertTriangle} items={intelligence.risks} title="Risks" empty="No material rota risk is currently detected." tone="risk" />
        <InsightColumn icon={ArrowRight} items={intelligence.recommendations} title="Recommendations" empty="No action is recommended." tone="action" />
      </div>

      <div className="operations-intelligence__facts">
        <span><Gauge size={16} /><strong>{intelligence.labourPct.toFixed(1)}%</strong> planned labour</span>
        <span><CircleDollarSign size={16} /><strong>{formatCurrency(plan.plannedCost)}</strong> planned cost</span>
      ...
      <section className="rota-copilot" aria-labelledby="rota-copilot-title">
        <div className="rota-copilot__heading">
          <div><MessageSquareText size={19} /><span><strong id="rota-copilot-title">Ask the rota copilot</strong><small>Scenario advice grounded in this saved rota suggestion</small></span></div>

  ...
  return (
    <div className="management-email-grid">
      <form action={settingsAction} className="report-form management-email-settings">
        ...
        <label className="source-confirmation source-confirmation--standalone"><input name="enabled" type="hidden" value="false" /><input defaultChecked={settings.enabled} name="enabled" type="checkbox" value="true" /><span>Automatically send the latest management pack each week.</span></label>
        <label className="source-confirmation source-confirmation--standalone"><input name="allowPartial" type="hidden" value="false" /><input defaultChecked={settings.allowPartial} name="allowPartial" type="checkbox" value="true" /><span>Send a clearly labelled partial pack if at least one kitchen is approved but another is still outstanding.</span></label>
      ...
      <div className="management-email-test-panel">
        ...
        <label className="field"><span className="field__label">Reporting week</span><select className="field__input" onChange={(event) => setSelectedPeriod(event.target.value)} value={selectedPeriod}>{periods.map((period) => <option key={period.id} value={period.id}>{period.label}</option>)}</select></label>
        <form action={testAction}><input name="periodId" type="hidden" value={selectedPeriod} /><button className="button button--secondary" disabled={testPending || !selectedPeriod} type="submit"><TestTube2 aria-hidden="true" size={16} />{testPending ? "Sending…" : "Send full test to me"}</button></form>
        {testState.status !== "idle" ? <ActionMessage state={testState} /> : null}
        <form action={liveAction} className="management-email-live-form">
          <input name="periodId" type="hidden" value={selectedPeriod} />
          <button className="button button--primary" disabled={livePending || !selectedPeriod || !settings.recipientEmail} type="submit"><Send aria-hidden="true" size={16} />{livePending ? "Sending…" : `Send selected pack to ${settings.recipientName.split(" ")[0] || "recipient"}`}</button>
        </form>

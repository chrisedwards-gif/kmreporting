...
export function ActionLogTable({ actions }: { actions: PerformanceActionItem[] }) {
  const [manager, setManager] = useState("all");
  const [status, setStatus] = useState("open");
  const [priority, setPriority] = useState("all");
  const managers = useMemo(
    () => [...new Map(actions.map((item) => [item.managerId, item.managerName])).entries()].sort((a, b) => a[1].localeCompare(b[1])),
    [actions],
  );
  const filtered = useMemo(() => actions.filter((item) => {
    const managerMatch = manager === "all" || item.managerId === manager;
    const statusMatch = status === "all"
      || (status === "open" && !["complete", "cancelled"].includes(item.status))
      || item.status === status;
  ...
  return (
    <>
      ...
      <div className="table-scroll">
        <table className="data-table action-log-table">
          <thead><tr><th>Manager</th><th>Action</th><th>Due</th><th>Priority</th><th>Status & outcome</th><th><span className="sr-only">Review</span></th></tr></thead>
          <tbody>
            {filtered.map((item) => (
              <tr key={item.id}>
                <td><strong>{item.managerName}</strong><div className="data-table__subtext">{item.siteName}</div></td>
                <td><strong>{item.action}</strong>{item.successMeasure ? <div className="data-table__subtext">Success: {item.successMeasure}</div> : null}</td>
                <td>{item.dueDate ? formatDate(item.dueDate) : "—"}</td>
                <td><span className={`priority-pill priority-pill--${item.priority}`}>{item.priority}</span></td>
                <td>
                  <form action={updateOwnManagerAction} className="action-status-form">
                    <input name="actionId" type="hidden" value={item.id} />
                    <select aria-label={`Status for ${item.action}`} className="field__input field__input--compact" defaultValue={item.status === "cancelled" ? "blocked" : item.status} name="status">
                      <option value="not_started">Not started</option><option value="in_progress">In progress</option><option value="blocked">Blocked</option><option value="complete">Complete</option>
                    </select>
                    <input aria-label={`Outcome for ${item.action}`} className="field__input field__input--compact" defaultValue={item.outcome} name="outcome" placeholder="Progress / outcome" />
                    <button className="button button--secondary button--compact" type="submit">Update</button>
                  </form>
                </td>
                <td>{item.reviewId ? <Link aria-label="Open source 1-1" href={`/one-to-ones/${item.reviewId}`}><ExternalLink aria-hidden="true" size={16} /></Link> : "—"}</td>
              </tr>
            ))}

...
export default async function NotificationsPage() {
  ...
  if (supabase) {
    ...
    if (error) {
      ...
      loadError = error.message.includes("one_to_one_review_id")
        ? "Apply migration 013 to load the expanded notification history. Test delivery remains available."
        : "Notification history could not be loaded. Test delivery is still available.";
    } else {
  ...
  const deliveryState = !environment.reminderWebhookUrl
    ...
    : environment.reminderRecipientOverride
      ...
      : environment.isPreview
        ? <><TriangleAlert aria-hidden="true" size={15} /> Preview webhook is live without a recipient override. Scheduled reminders and finalised 1-1s use profile email addresses.</>
        : <><CheckCircle2 aria-hidden="true" size={15} /> Delivery webhook configured.</>;

  return (
    <>
      <header className="page-header"><div><p className="page-header__eyebrow">Delivery control</p><h1 className="page-header__title">Reminders & notifications.</h1><p className="page-header__copy">Monitor report reminders, approval prompts and finalised 1-1 summaries from one delivery log.</p></div></header>
      {loadError ? <div className="form-message form-message--error" role="alert">{loadError}</div> : null}
      <div className="dashboard-grid">
        <section className="panel"><div className="panel__header"><div><h2 className="panel__title">Test delivery</h2><p className="panel__subtitle">Tests go only to your profile email or the configured UAT override</p></div><BellRing aria-hidden="true" size={19} /></div><div className="panel__body"><div className="privacy-callout notification-delivery-state">{deliveryState}</div><TestNotificationForm /></div></section>
      ...
      <section className="panel notifications-history"><div className="panel__header"><div><h2 className="panel__title">Delivery history</h2><p className="panel__subtitle">Reminders, tests and manager 1-1 summaries</p></div></div><div className="panel__body"><div className="report-list">

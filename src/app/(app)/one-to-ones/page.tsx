...
export default async function OneToOnesPage({ searchParams }: { searchParams: Promise<{ manager?: string }> }) {
  ...
  const canManage = ["admin", "group_manager"].includes(profile.actualRole);
  ...
  return (
    <>
      <header className="page-header"><div><p className="page-header__eyebrow">Performance</p><h1 className="page-header__title">{profile.actualRole === "kitchen_manager" ? "My 1-1s." : "Manager 1-1s."}</h1><p className="page-header__copy">Review previous meetings, current priorities and the actions that carry forward until complete.</p></div><div className="page-header__actions"><Link className="button button--secondary" href="/performance/actions"><ListChecks aria-hidden="true" size={15} /> Action log</Link>{profile.actualRole !== "kitchen_manager" ? <Link className="button button--secondary" href="/performance/probation"><Scale aria-hidden="true" size={15} /> Probation</Link> : null}{profile.actualRole === "admin" ? <Link className="button button--secondary" href="/performance/managers"><UserRoundCog aria-hidden="true" size={15} /> People & access</Link> : null}</div></header>
      {profile.isAccessPreview ? <div className="privacy-callout">Admin site mode for {profile.previewSiteName}. You can start, edit, finalise, reopen and acknowledge this manager&apos;s 1-1s.</div> : null}

...
export default async function OneToOnesPage({ searchParams }: { searchParams: Promise<{ manager?: string }> }) {
  ...
  const selectedManager = profile.scopeManagerId ?? manager;
  const [allManagers, allReviews, allTrends] = await Promise.all([getManagers(), getOneToOnes(selectedManager), getPerformanceTrends(selectedManager)]);
  const managers = allManagers.filter((item) => scopeContainsSite(profile.siteScopeIds, item.siteId) && (!selectedManager || item.id === selectedManager));
  const reviews = allReviews.filter((item) => scopeContainsSite(profile.siteScopeIds, item.siteId) && (!profile.scopeManagerId || item.managerId === profile.scopeManagerId));
  const visibleSiteNames = new Set(managers.map((item) => item.siteName));
  ...
  return (
    <>
      <header className="page-header"><div><p className="page-header__eyebrow">Performance</p><h1 className="page-header__title">{profile.navigationRole === "kitchen_manager" ? "My 1-1s." : "Manager 1-1s."}</h1><p className="page-header__copy">Review previous meetings, current priorities and the actions that carry forward until complete.</p></div><div className="page-header__actions"><Link className="button button--secondary" href="/performance/actions"><ListChecks aria-hidden="true" size={15} /> Action log</Link>{profile.navigationRole !== "kitchen_manager" ? <Link className="button button--secondary" href="/performance/probation"><Scale aria-hidden="true" size={15} /> Probation</Link> : null}{profile.capabilities.admin && !profile.isAccessPreview ? <Link className="button button--secondary" href="/performance/managers"><UserRoundCog aria-hidden="true" size={15} /> People & access</Link> : null}</div></header>
      {profile.isAccessPreview ? <div className="privacy-callout">Admin site mode for {profile.previewSiteName}. Only {profile.previewManagerName ?? "the assigned manager"} and this kitchen’s 1-1 history are loaded.</div> : null}
      ...
      <div className="manager-grid">{managers.map((managerRecord) => {
        ...
        const week = getAvailableOneToOneWeek(managerRecord);
        const currentWeekReview = week ? assignmentReviews.find((review) => review.weekCommencing === week.start) : null;
        return <section className="panel manager-card" key={managerRecord.assignmentId}><div className="panel__header"><div><h2 className="panel__title">{managerRecord.fullName}</h2><p className="panel__subtitle">{managerRecord.roleTitle} · {managerRecord.siteName}</p></div>{latestScored?.overallScore != null ? <span className={`score-pill score-pill--${scoreRag(latestScored.overallScore)}`}>{latestScored.overallScore.toFixed(1)}</span> : null}</div><div className="panel__body"><div className="manager-card__identity"><Link2 aria-hidden="true" size={14} /> One login identity · assigned from {formatDate(managerRecord.assignmentStartsOn)}</div>{managerRecord.focusAreas.length ? <div className="manager-card__focus">{managerRecord.focusAreas.slice(0, 6).map((area) => <span className="source-chip" key={area}>{area}</span>)}</div> : null}<div className="manager-card__stats"><span><strong>{openActions.length}</strong> open actions</span><span className={overdue ? "manager-card__stat--overdue" : ""}><strong>{overdue}</strong> overdue</span><span><strong>{managerReviews.length}</strong> reviews</span></div>{currentWeekReview ? <Link className="button button--secondary" href={`/one-to-ones/${currentWeekReview.id}`}><ClipboardCheck aria-hidden="true" size={16} /> Open this week’s 1-1</Link> : canManage && week ? <><Link className="button button--primary" href={`/one-to-ones/new?assignment=${managerRecord.assignmentId}`}><CalendarPlus aria-hidden="true" size={16} /> {week.isComplete ? `Start 1-1 for w/c ${formatDate(week.start)}` : "Start 1-1 now"}</Link>{!week.isComplete ? <div className="privacy-callout">The meeting can be recorded now. Weekly KPI figures will populate when the kitchen report is available.</div> : null}</> : week ? <div className="privacy-callout">The next review will appear here after group management starts it.</div> : <div className="privacy-callout">This manager assignment is not currently active.</div>}</div></section>;
      })}{!managers.length ? <section className="panel empty-state"><Flame aria-hidden="true" size={22} /><h2>No manager assignment is available in this workspace.</h2><p>Assign a primary manager in Sites & access before creating their 1-1 history.</p></section> : null}</div>
      <section className="panel"><div className="panel__header"><div><h2 className="panel__title">Performance trend</h2><p className="panel__subtitle">Overall score from finalised reviews</p></div></div><div className="panel__body"><PerformanceTrendChart points={trends} /></div></section>

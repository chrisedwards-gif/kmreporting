...
export default async function OneToOnesPage({ searchParams }: { searchParams: Promise<{ manager?: string }> }) {
  ...
  const selectedManager = profile.isAccessPreview ? profile.previewManagerId ?? undefined : profile.actualRole === "kitchen_manager" ? profile.id : manager;
  ...
  const isManagerWorkspace = profile.navigationRole === "kitchen_manager";
  ...
  return (
    <>
      <header className="page-header"><div><p className="page-header__eyebrow">Performance</p><h1 className="page-header__title">{isManagerWorkspace ? "My 1-1s." : "Manager 1-1s."}</h1><p className="page-header__copy">Review previous meetings, current priorities and the actions that carry forward until complete.</p></div>{!isManagerWorkspace ? <div className="page-header__actions"><Link className="button button--secondary" href="/performance/actions"><ListChecks aria-hidden="true" size={15} /> Action log</Link><Link className="button button--secondary" href="/performance/probation"><Scale aria-hidden="true" size={15} /> Probation</Link>{profile.capabilities.admin ? <Link className="button button--secondary" href="/performance/managers"><UserRoundCog aria-hidden="true" size={15} /> People & access</Link> : null}</div> : null}</header>
      {profile.isAccessPreview ? <div className="privacy-callout">Kitchen Manager view for {profile.previewSiteName}. The page matches the assigned manager&apos;s workspace while your Admin capabilities let you start, edit and finalise records.</div> : null}

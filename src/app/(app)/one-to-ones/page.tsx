  ...
  return (
    <>
      <header className="page-header">
        <div><p className="page-header__eyebrow">Performance</p><h1 className="page-header__title">Manager 1-1s.</h1><p className="page-header__copy">Drafts remain resumable, finalised reviews build the performance history, and managers can sign in to review, acknowledge and update their actions.</p></div>
        <div className="page-header__actions"><Link className="button button--secondary" href="/performance/actions"><ListChecks aria-hidden="true" size={15} /> Action log</Link>{profile.role !== "kitchen_manager" ? <Link className="button button--secondary" href="/performance/probation"><Scale aria-hidden="true" size={15} /> Probation</Link> : null}{profile.role === "admin" ? <Link className="button button--secondary" href="/performance/managers"><UserRoundCog aria-hidden="true" size={15} /> Manager admin</Link> : null}</div>
      </header>

      {openDrafts.length ? <section className="panel panel--attention"><div className="panel__header"><div><h2 className="panel__title">Open drafts</h2><p className="panel__subtitle">Saved work that can be continued without starting again</p></div><span className="status-badge status-badge--draft">{openDrafts.length} open</span></div><div className="report-list">{openDrafts.map((review) => <Link className="report-row report-row--slim" href={`/one-to-ones/${review.id}`} key={review.id}><div className="site-cell"><div className="site-cell__mark">{review.managerName.split(" ").map((part) => part[0]).join("").slice(0, 2)}</div><div><div className="site-cell__name">{review.managerName}</div><div className="site-cell__manager">{review.siteName} · w/c {formatDate(review.weekCommencing)}</div></div></div><div><span className="report-row__metric-label">Progress</span>{review.overallScore === null ? "Not finalised" : review.overallScore.toFixed(1)}</div><span className="button button--secondary button--compact"><ClipboardCheck aria-hidden="true" size={14} /> Continue draft</span></Link>)}</div></section> : null}

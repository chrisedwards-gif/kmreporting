...
import { reopenOneToOne } from "@/app/actions/one-to-ones";
import { ManagerAcknowledgementForm } from "@/components/one-to-ones/manager-acknowledgement-form";
...
export default async function OneToOneDetailPage({ params }: { params: Promise<{ reviewId: string }> }) {
  ...
  return (
    <>
      ...
      {canAcknowledge ? <section className="panel panel--attention"><div className="panel__header"><div><h2 className="panel__title">Manager review</h2><p className="panel__subtitle">Read the full record and action points below, then acknowledge it</p></div></div><div className="panel__body"><ManagerAcknowledgementForm reviewId={detail.id} /></div></section> : null}
      {detail.status === "acknowledged" ? <section className="panel acknowledgement-record"><div className="panel__header"><div><h2 className="panel__title">Manager acknowledgement</h2><p className="panel__subtitle">Permanent acknowledgement trail{detail.acknowledgedByName ? ` · ${detail.acknowledgedByName}` : ""}{detail.acknowledgedAt ? ` · ${formatDate(detail.acknowledgedAt)}` : ""}</p></div></div><div className="panel__body"><p>{detail.managerResponse || "Acknowledged without an additional comment."}</p></div></section> : null}
      <OneToOneForm assignmentId={detail.assignmentId} detail={detail} initialActions={reviewActions} kpis={kpis} managerFirstName={detail.managerName.split(" ")[0]} managerName={detail.managerName} openActions={openActions.filter((action) => siteIsInScope(profile.siteScopeIds, action.siteId))} weekCommencing={detail.weekCommencing} />
    </>
  );
}

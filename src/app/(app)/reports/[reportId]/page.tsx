...
import { getReportingBundle, roleCanApprove } from "@/lib/data/reporting";
...
export default async function ReportDetailPage({ params }: { params: Promise<{ reportId: string }> }) {
  ...
  const canEditDraft = Boolean(profile && ["admin", "group_manager", "kitchen_manager"].includes(profile.actualRole));
  const canUploadSales = Boolean(canEditDraft && ["draft", "submitted", "review_required"].includes(report.status));
  const canApprove = Boolean(profile && roleCanApprove(profile.actualRole));
  ...
  return (
    <>
      ...
      {profile?.isAccessPreview ? <div className="privacy-callout" style={{ marginBottom: "1rem" }}>Admin site mode for {profile.previewSiteName}. You retain full report editing and approval controls.</div> : null}

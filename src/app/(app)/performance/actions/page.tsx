...
import { CreateActionForm } from "@/components/performance/create-action-form";
import { ActionLogTable } from "@/components/performance/action-log-table";
import { requireRole } from "@/lib/auth/dal";
import { siteIsInScope } from "@/lib/auth/site-scope";
import { getPerformanceActionTargets } from "@/lib/data/action-log";
...
export default async function PerformanceActionsPage() {
  ...
  const [rawActions, rawTargets] = await Promise.all([getPerformanceActions(), getPerformanceActionTargets()]);
  ...
  const targets = filterPerformanceActionTargets(rawTargets, profile.siteScopeIds, profile.scopeManagerId);

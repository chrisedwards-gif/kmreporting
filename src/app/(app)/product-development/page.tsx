...
export default async function ProductDevelopmentPage() {
  ...
  const launchCount = items.filter((item) => Boolean(item.targetLaunchDate)).length;
  ...
  return (
    <>
      ...
      <section className="metric-grid metric-grid--three" aria-label="Product development summary">
        ...
        <article className="metric-card"><Rocket aria-hidden="true" size={21} /><div className="metric-card__value">{launchCount}</div><div className="metric-card__note">With launch dates · {liveCount} already live</div></article>
      </section>

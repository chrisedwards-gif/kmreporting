...
export default async function RotaFeedbackPage({ searchParams }: { searchParams: Promise<Params> }) {
  ...
  const labourQuery = profile.actualRole === "kitchen_manager"
    ? admin.from("rota_daily_labour_history").select("business_date,scheduled_hours,actual_hours,scheduled_hourly_cost,actual_hourly_cost,imported_at")
    : admin.from("rota_daily_labour_history").select("business_date,scheduled_hours,actual_hours,scheduled_hourly_cost,actual_hourly_cost,salary_cost_allocated,imported_at");
  const [feedbackResult, salesResult, labourResult] = await Promise.all([
    ...
    labourQuery.eq("site_id", selectedSite.id).gte("business_date", fromDate).lte("business_date", businessDate).order("imported_at"),
  ]);
  ...
  const canSeeSalaryTotals = profile.actualRole === "admin" || profile.actualRole === "group_manager";
    ...
    <>
      <header className="page-header">
        ...
        <div className="page-header__actions">
          <Link className="button button--secondary" href={`/rotas?site=${selectedSite.id}`}><ArrowLeft aria-hidden="true" size={16} /> Back to rota builder</Link>
        </div>
      </header>
        ...
        <section className="stack">
          <article className="form-section">
            ...
            <div className="cost-summary">
              ...
              {canSeeSalaryTotals ? <EvidenceRow label="Allocated salary cost" value={labourByDate.has(businessDate) ? formatCurrency(numberValue(labourByDate.get(businessDate)?.salary_cost_allocated)) : "Not imported"} /> : null}
            </div>
          </article>
          <div className="privacy-callout">Individual pay remains private. Kitchen managers see operational hours and hourly-team totals only; salary allocation is restricted to group management.</div>
        </section>
      </div>

  ...
  return (
    <>
      <header className="page-header">
        ...
        <div className="page-header__actions">{site ? <Link className="button button--secondary" href={`/rotas/feedback?site=${site.id}`}><MessageSquareText aria-hidden="true" size={16} /> Shift feedback</Link> : null}{canManageTeam ? <><Link className="button button--secondary" href={site ? `/rotas/settings?site=${site.id}` : "/rotas/settings"}><Settings2 aria-hidden="true" size={16} /> Calibrate site</Link><Link className="button button--secondary" href="/rotas/team"><UsersRound aria-hidden="true" size={16} /> Staff profiles</Link></> : null}</div>
      </header>
      <form className="rota-filters panel" method="get"><label className="field"><span className="field__label">Kitchen</span><select className="field__input" defaultValue={site?.id} name="site">{workspace.sites.map((item) => <option key={item.id} value={item.id}>{item.name} · {item.labourTarget.toFixed(1)}% labour</option>)}</select></label><label className="field"><span className="field__label">Week commencing</span><input className="field__input" defaultValue={workspace.weekStart} name="week" type="date" /></label><button className="button button--secondary" type="submit"><CalendarClock aria-hidden="true" size={16} /> Load week</button><div className="rota-filters__weeks"><Link aria-label="Previous week" className="icon-button" href={`/rotas?week=${priorWeek}${siteQuery}`}><ArrowLeft aria-hidden="true" size={16} /></Link><span>{formatDate(workspace.weekStart)}–{formatDate(addDays(workspace.weekStart, 6))}</span><Link aria-label="Next week" className="icon-button" href={`/rotas?week=${nextWeek}${siteQuery}`}><ArrowRight aria-hidden="true" size={16} /></Link></div></form>
      {workspace.error ? <div className="form-message form-message--error" role="alert">{workspace.error}</div> : null}
      {site ? <><section aria-label="Rota readiness" className="rota-readiness"><Readiness icon={DatabaseZap} label="Sales history" ready={historyWeeks >= workspace.minimumHistoryWeeks} value={`${historyWeeks} full week${historyWeeks === 1 ? "" : "s"}`} detail={`Minimum ${workspace.minimumHistoryWeeks}; ${workspace.forecastWeeks} matching weekdays used`} /><Readiness icon={UsersRound} label="Staff constraints" ready={workspace.staff.length >= 2} value={`${workspace.staff.length} active profile${workspace.staff.length === 1 ? "" : "s"}`} detail="Wages remain private; availability and skills drive assignment" /><Readiness icon={Settings2} label="Day-part demand" ready={hasHourlyShape} value={hasHourlyShape ? "Actual hourly sales" : "Editable template"} detail={hasHourlyShape ? "Peak curve learned from EPOS" : "Clearly labelled until hourly EPOS data arrives"} /><Readiness icon={ShieldCheck} label="RotaCloud" ready={workspace.rotacloudConfigured} value={workspace.rotacloudConfigured ? "Read-only sync enabled" : "Manual handoff"} detail="No shifts are published automatically in this release" /></section><RotaControls siteId={site.id} weekStart={workspace.weekStart} />{workspace.events.length ? <section className="rota-events panel"><strong>Forecast adjustments this week</strong>{workspace.events.map((event) => <span key={`${event.eventDate}-${event.title}`}>{formatDate(event.eventDate)} · {event.title} · {event.salesUpliftPct >= 0 ? "+" : ""}{event.salesUpliftPct}%</span>)}</section> : null}{plan ? <RotaPlanView aiReview={aiReview} plan={plan} signals={signals} staffTargets={staffTargets} /> : <section className="panel empty-state"><CalendarClock aria-hidden="true" size={28} /><h2>No suggestion generated for this week.</h2><p>Complete readiness gaps, add known events, then generate.</p></section>}</> : null}
    </>
  );
}

...
export default async function RotasPage({ searchParams }: { searchParams: Promise<Params> }) {
  ...
  const demoPlan = environment.isDemo && site && workspace.staff.length
    ...
    : null;
  const plan = workspace.latestPlan ?? (demoPlan
  ...
  const hasTeam = workspace.staff.length >= 2;
  ...
  const readiness = [
    ...
    },
    {
      ...
      detail: "Visible on the rota heatmap",
      ready: true,
    },
    {
      label: "RotaCloud handoff",
      value: workspace.rotacloudConfigured ? "Read-only connection" : "CSV export",
      detail: "Nothing is published automatically",
      ready: true,
    },
  ];
    ...
    <>
      <header className="rota-page-header">
        <div>
          ...
          <h1>Plan next week’s rota</h1>
          <p>See when the kitchen will be busy, move the shifts into place and fix anything highlighted before you export.</p>
        </div>
        <nav aria-label="Rota tools" className="rota-page-header__links">
          {site ? (
            <Link className="button button--secondary" href={`/rotas/feedback?site=${site.id}`}>
          ...
          {canManageTeam ? (
            <>
              ...
              <Link className="button button--secondary" href={site ? `/rotas/settings?site=${site.id}` : "/rotas/settings"}>
      ...
      <form className="rota-toolbar panel" method="get">
        ...
        <label className="field">
          ...
          <input className="field__input" defaultValue={workspace.weekStart} name="week" type="date" />
        </label>
        <button className="button button--secondary" type="submit">
          <CalendarClock aria-hidden="true" size={16} /> Open week
        </button>
        <div className="rota-toolbar__week">
          <Link aria-label="Previous week" className="icon-button" href={`/rotas?week=${priorWeek}${siteQuery}`}>
            <ArrowLeft aria-hidden="true" size={16} />
          </Link>
          <span>{formatDate(workspace.weekStart)}–{formatDate(addDays(workspace.weekStart, 6))}</span>
          <Link aria-label="Next week" className="icon-button" href={`/rotas?week=${nextWeek}${siteQuery}`}>
      ...
      {workspace.error ? (
        <div className="form-message form-message--error" role="alert">{workspace.error}</div>
      ) : null}
        ...
        <>
          <details className={`rota-setup panel ${readyCount < readiness.length ? "rota-setup--attention" : ""}`}>
            <summary>
              ...
              <span>
                <strong>{readyCount === readiness.length ? "Everything needed to plan is ready" : `${readyCount} of ${readiness.length} planning inputs are ready`}</strong>
                <small>Open this only when you need to check where the suggestion comes from.</small>
              </span>
            </summary>
          ...
          {plan ? (
            ...
            <section className="panel empty-state">
              ...
              <h2>No rota has been built for this week yet</h2>
              <p>Use “Build this week’s rota” above. You will get an editable starting point, not a published rota.</p>
            </section>
          )}
        </>
      ) : null}

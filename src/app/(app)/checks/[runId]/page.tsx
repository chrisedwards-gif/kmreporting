...
export default async function KitchenCheckDetailPage({ params }: { params: Promise<{ runId: string }> }) {
  ...
  return (
    <>
      <header className="page-header"><div><p className="page-header__eyebrow">{detail.siteName} · {detail.cadence} · v{detail.templateVersion}</p><h1 className="page-header__title">{detail.templateName}.</h1><p className="page-header__copy">{formatDate(detail.periodStart)}{detail.periodEnd !== detail.periodStart ? ` – ${formatDate(detail.periodEnd)}` : ""} · {detail.description}{detail.submittedAt ? <> · Completed by {detail.completedByName ?? "Team member"} on <time dateTime={detail.submittedAt}>{formatDateTime(detail.submittedAt)}</time></> : null}</p></div><span className={`status-badge status-badge--${detail.result === "pass" ? "approved" : detail.result === "watch" ? "review_required" : detail.result === "fail" ? "returned" : "draft"}`}>{detail.status.replaceAll("_", " ")} · {detail.result.replaceAll("_", " ")}</span></header>

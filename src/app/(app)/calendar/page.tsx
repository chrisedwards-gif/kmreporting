...
export default async function CalendarPage() {
  ...
  const [visibleLinks, rawAdminData] = await Promise.all([
    getTeamupCalendarLinks(profile),
    canConfigure ? getTeamupCalendarAdminData() : Promise.resolve({ links: [], sites: [] }),
  ]);
  const adminData = profile.siteScopeIds === null ? rawAdminData : {
    links: rawAdminData.links.filter((link) => siteIsInScope(profile.siteScopeIds, link.siteId)),
    sites: rawAdminData.sites.filter((site) => profile.siteScopeIds?.includes(site.id)),
  };
  ...
  return (
    <>
      ...
      {profile.isAccessPreview ? <div className="privacy-callout" style={{ marginBottom: "1rem" }}>Only {profile.previewSiteName} and any intentionally group-wide calendar are visible in this workspace.</div> : null}
      ...
      {visibleLinks.length > 1 ? <section className="panel"><div className="panel__header"><div><h2 className="panel__title">Available calendars</h2><p className="panel__subtitle">Group-wide and kitchen-specific links in this workspace</p></div></div><div className="panel__body action-list">{visibleLinks.map((link) => <a className="action-list__row" href={link.calendarUrl} key={link.id} rel="noreferrer" target="_blank"><div><strong>{link.title}</strong><span>{link.siteName}</span></div><ExternalLink aria-hidden="true" size={15} /></a>)}</div></section> : null}
        ...
        <section className="panel">
          ...
          <div className="panel__body dashboard-grid dashboard-grid--balanced">
            ...
            <div className="stack">
              ...
              {!adminData.links.length ? <div className="empty-inline">No calendar links configured in this workspace.</div> : null}
            </div>
          </div>
        </section>
      ) : null}

...
export async function GET(request: NextRequest) {
  ...
  lines.push(`- Active item-cost rows: ${intelligence.dataCoverage.menuCostRows}`);
  lines.push(`- Hourly EPOS rows: ${intelligence.dataCoverage.hourlySalesRows}`);
  lines.push(`- Hourly labour rows: ${intelligence.dataCoverage.hourlyLabourRows}`);
  ...
  lines.push(`Produce a concise Group Chef weekly report with: (1) the five highest-value decisions, (2) what changed and likely drivers, (3) menu/pricing actions including contribution-margin evidence where available, (4) labour/daypart actions using exact hourly staffing only where hourly labour coverage exists, (5) purchasing/food-cost/waste actions, (6) data-quality exceptions, (7) accepted/measuring actions that need follow-up, and (8) a next-week measurement plan. Do not recommend permanent menu or staffing changes from weak samples. Clearly label tests, hypotheses and confirmed findings.`);
  lines.push(``);
  lines.push(`---`);

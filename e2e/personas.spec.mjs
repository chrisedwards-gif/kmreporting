...
test("Admin upload route shows exact Group Chef report downloads", async ({ page }) => {
  ...
  await expect(page.getByText(/Sales Reports → End Of Week Report/).first()).toBeVisible();
  await expect(page.getByText(/Reporting → Goods Purchased/).first()).toBeVisible();
  await expect(page.getByText(/Reporting → Credits Overview/).first()).toBeVisible();
  await expect(page.getByText(/Reports → Hours & Costs → Daily Totals/).first()).toBeVisible();
